  ...
  return (
    <ul className="max-w-xs mx-auto">
      {tasks.map((task) => (
        <li
          key={task.id}
          className="flex items-center justify-between text-cyan-100 rounded-md p-4 mb-4"
        >
          <div className="border-2 p-2">
            <div className="flex gap-4 justify-center">
              ...
              >
                <IoIosClose className="text-red-300" size={26} />
              </button>
            </div>
            ...
            {editingTask === task.id ? (
              <form
                onSubmit={(e) => {
                  e.preventDefault();
                  handleEdit(task.id, editedTitle, editedDescription);
                }}
